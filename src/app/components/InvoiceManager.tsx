...
type InvoiceType = "invoice" | "proforma" | "receipt";
...
export default function InvoiceManager({ theme = "dark" }: { theme?: "dark" | "light" }) {
  const [tab, setTab] = useState<"list" | "create" | "clients" | "overdue">("list");
  const [invoices, setInvoices] = useState<Invoice[]>(INIT_INVOICES);
  const [expanded, setExpanded] = useState<string | null>(null);
  const [statusFilter, setStatusFilter] = useState<"all" | InvoiceStatus>("all");
  const [invType, setInvType] = useState<InvoiceType>("invoice");
  ...
  const recordPayment = () => {
    ...
    setInvoices(prev => prev.map(inv => {
      ...
      const newStatus: InvoiceStatus = newPaid >= inv.total ? "paid" : newPaid > 0 ? "partial" : inv.status;
  ...
  const updateItem = (i: number, field: keyof InvoiceItem, val: string | number) => setItems(prev => prev.map((item,idx) =>
      ...
      <div className={`inv-root ${theme}`}>
        ...
        <div className="inv-tabs">
          {[["list","📋 All Invoices"],["create","✏️ Create"],["clients","👤 Clients"],["overdue","⚠️ Overdue"]].map(([k,l]) => (
            <button key={k} className={`inv-tab ${tab===k?"active":""}`} onClick={() => setTab(k as typeof tab)}>{l}</button>
          ))}
        </div>
          ...
          <>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:8, marginBottom:14 }}>
              {[
                ...
                <div className="inv-stat" key={l as string}>
                      ...
                      <div style={{ display:"flex", gap:7, marginTop:12, flexWrap:"wrap" }}>
                        ...
                        {inv.status === "draft" && (
                          <button className="inv-btn" style={{ fontSize:11, padding:"6px 12px", background:"#3b82f6" }}
                            onClick={() => setInvoices(prev => prev.map(i => i.id === inv.id ? {...i, status:"sent" as InvoiceStatus} : i))}>
                            📤 Mark Sent
                          </button>
                        )}
                      </div>
          ...
          <>
            ...
            <div style={{ display:"flex", background:"var(--inp)", borderRadius:9, overflow:"hidden", marginBottom:14 }}>
              {[["invoice","📄 Tax Invoice"],["proforma","📋 Proforma"],["receipt","🧾 Receipt"]].map(([k,l]) => (
                <button key={k} onClick={() => setInvType(k as InvoiceType)}
